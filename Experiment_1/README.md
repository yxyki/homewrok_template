...
## 实验预期结果与实际结果
+ **实验预期结果：** 以n个随机数为例,在*最坏情况*下，可以预测实验结果：BubleSort比较次数为n*(n-1)/2次，MergeSort比较次数为nlogn，而QuickSort的比较次数为n*(n-1)/2次。而在*平均情况*下，BubleSort的时间复杂度为*O(n^2)*,MergeSort的时间复杂度为*O(nlogn)*,QuickSort的时间复杂度为*O(nlogn)*,因此可以预测三种的基本比较次数也是如此。
...
>通过分析曲线以及表格可知，快速排序以及合并排序的比较次数没有太大的区别，因此可以推测属于同一个时间复杂度，而冒泡排序则远远大于前两者，应属于更大一个级别的时间复杂度。时间复杂度可以直观地体现出算法的数据输入随数据集的增加的变化情况。